...
function updateRoute(id) {
    ...
    for (var i = 0; i < data.routes.length; i++) {
        if (data.routes[i].id === id) {
            ...
            // для записи журнала
            addHistory('Редактирование', id, task);
...
function deleteRoute(id) {
    ...
    if (confirm('Удалить?'))
    //для записи журнала
     addHistory('Удаление', id, '');   
    { data.routes = data.routes.filter(function(r){return r.id !== id;}); saveData(); renderTable(); }
}
...
function screenshotTable() {
    ...
    for (var r = 0; r < vis.length; r++) {
        ...
        for (var c = 0; c < 8; c++) {
            var text = cells[c] ? cells[c].textContent.trim() : '';
           var nowrap = c <= 2 ? 'white-space:nowrap;' : 'white-space:normal;overflow-wrap:break-word;word-break:break-all;';
...
function addHistory(action, routeId, details) {
